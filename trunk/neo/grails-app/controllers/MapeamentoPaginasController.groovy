 ...
 */
class MapeamentoPaginasController {
	...
	def list = {
		...
		[requestmapList: MapeamentoPaginas.list(params)]
	}

	def show = {
		def requestmap = MapeamentoPaginas.get(params.id)
		if (!requestmap) {
			flash.message = "MapeamentoPaginas not found with id $params.id"
	...
	def delete = {
		...
		flash.message = "MapeamentoPaginas $params.id deleted."
		redirect(action: list)
	}
	...
	def update = {
		...
		if (requestmap.version > version) {
			requestmap.errors.rejectValue 'version', "requestmap.optimistic.locking.failure",
				"Another user has updated this MapeamentoPaginas while you were editing."
	...
	def create = {
		[requestmap: new MapeamentoPaginas(params)]
	}
	 ...
	 */
	def save = {
		def requestmap = new MapeamentoPaginas(params)
		if (requestmap.save()) {
			authenticateService.clearCachedRequestmaps()
			redirect action: show, id: requestmap.id
		}
