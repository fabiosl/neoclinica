...
class HierarquiaController {
	...
	def list = {
		...
		[authorityList: Hierarquia.list(params)]
	}
	 ...
	 */
	def show = {
		def authority = Hierarquia.get(params.id)
		if (!authority) {
			flash.message = "Hierarquia not found with id $params.id"
	...
	def delete = {
		...
		flash.message = "Hierarquia $params.id deleted."
		redirect action: list
	}
	...
	def update = {
		...
		if (authority.version > version) {
			authority.errors.rejectValue 'version', 'authority.optimistic.locking.failure',
				'Another user has updated this Hierarquia while you were editing.'
